...
feature_map = {name: feature_translation.get(name, name) for name in feature_names}
importance_df_xgb = pd.DataFrame([{'feature': feature_map.get(k, k), 'importance': v} for k, v in score_dict.items()])

importance_df_xgb = importance_df_xgb.sort_values(by='importance', ascending=False)

importance_df_xgb['importance_norm'] = importance_df_xgb['importance'] / importance_df_xgb['importance'].sum()

print("特征映射：", feature_map)
print("特征重要性数据：", importance_df_xgb)
...
print(f"XGBoost 模型 - 准确率: {accuracy_xgb:.4f}")
print(f"XGBoost 模型 - 查准率: {precision_xgb:.4f}")
print(f"XGBoost 模型 - 召回率: {recall_xgb:.4f}")
print(f"XGBoost 模型 - F1 分数: {f1_xgb:.4f}")
print(f"XGBoost 模型 - 预测结果（新数据）: {mypre_binary}")
